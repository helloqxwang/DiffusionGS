...
def run(opt,optim):
    ...
    # )
    model = GaussianModel(3,bound=opt.bound)
    # import pdb;pdb.set_trace()
    model.random_init()
    model.training_setup(optim)
    criterion = torch.nn.MSELoss(reduction='none')

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    torch.autograd.set_detect_anomaly = True
    # opt.test=True
    opt.test_mode.append('circle_paths')
    # opt.test_mode.append('gt_paths')

    if opt.test:
        trainer = Trainer('gaussian', opt, model, device=device, workspace=opt.workspace, criterion=criterion,
                          fp16=False, metrics=[PSNRMeter()], use_checkpoint=opt.ckpt)
        ...
        if len(train_loader) < min_steps_per_epoch:
            train_loader = MultiLoader(loader=train_loader, num_repeats=1)
        ...
        trainer = Trainer('gaussian', opt, model, device=device, workspace=opt.workspace, optimizer=model.optimizer,
                          criterion=criterion, ema_decay=0.95, fp16=False, lr_scheduler=None,

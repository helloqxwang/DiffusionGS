...
def convert2CamInfo(poses, intrinsics, H, W):
    if poses.ndim == 2:
        R = poses[:3, :3]
        T = poses[:3, 3]
    else:
        R = poses[0, :3, :3]
        T = poses[0, :3, 3]
    return CameraInfo(uid=0, R=R, T=T, 
                            FovY=intrinsics[1], FovX=intrinsics[0],
                            image=0, image_path='', image_name='',                         
                            width=W, height=H)
...
class PatchRegulariser:
    ...
    def __init__(self, pose_generator: PatchPoseGenerator, patch_diffusion_model: GaussianDiffusion,
        ...
        self.H = 0
        self.W = 0
        self._img_intrinsics = full_image_intrinsics
    ...
    def get_diffusion_loss_with_rendered_patch(self, model, time,epoch) -> PatchOutputs:
        while True:
            try:
                ...
                if(epoch % 50 ==0):
                    # import pdb;pdb.set_trace()
                    depth_patch_np = depth_patch.detach().cpu().numpy()[0]
                    rgb_patch_np = rgb_patch.detach().cpu().numpy()[0]
                    output_depth = render_outputs["depth"].detach().cpu().permute(1,2,0).numpy()
                    output_rgb = render_outputs["render"].detach().cpu().permute(1,2,0).numpy()
                    cv2.imwrite(f'./patch_output/depth_patch_{epoch}.jpg', (depth_patch_np/20. * 255).astype(np.uint8)) 
                    cv2.imwrite(f'./patch_output/rgb_patch_{epoch}.jpg', cv2.cvtColor((rgb_patch_np * 255).astype(np.uint8), cv2.COLOR_RGB2BGR)) 
                    cv2.imwrite(f'./patch_output/output_depth_{epoch}.jpg', (output_depth/20. * 255).astype(np.uint8))
                    cv2.imwrite(f'./patch_output/output_rgb_{epoch}.jpg', cv2.cvtColor((output_rgb * 255).astype(np.uint8), cv2.COLOR_RGB2BGR)) 
    ...
    def get_diffusion_loss_with_sampled_patch(self, model , time, image, image_intrinsics,
                                              pose,epoch,camera_info) -> PatchOutputs:
        ...
        while True:
            try:
                depth_patch, rgb_patch, render_outputs = self._sample_patch(
                    image=image, image_intrinsics=image_intrinsics, pose=pose, model=model,camera=camera_info,epoch=epoch
                )
                if(epoch % 50 ==0):
                    depth_patch_np = depth_patch.detach().cpu().numpy()[0]
                    rgb_patch_np = rgb_patch.detach().cpu().numpy()[0]
    ...
    def get_loss_for_patch(self, depth_patch, rgb_patch, time, render_outputs,
        ...
        if self._diffusion_model.channels == 4:
            if rgb_patch.shape[-1] == 3:
                patch = torch.cat([rgb_patch.squeeze(), disparity_patch.squeeze()[..., None]], dim=-1)
            elif rgb_patch.shape[-3] == 3:
                patch = torch.cat([rgb_patch.squeeze().permute(1, 2, 0), disparity_patch.squeeze()[..., None]], dim=-1)
            else:
                raise ValueError('RGB patch has unexpected shape')
            patch = patch[None, ...]
            # from pdb import set_trace; set_trace()
    ...
    def _get_pseudo_intrinsics(self,intrinsics):
    ...
    def _render_random_patch(self, model):
        ...
        # print('Pose', pose)
        intrinsics = self._get_random_patch_intrinsics(self._img_intrinsics)
        ...
        pred_depth, pred_rgb, _, render_outputs = self._render_patch_with_intrinsics(intrinsics=intrinsics,
                                                                                     pose=pose, model=model,H=self._img_intrinsics.height,
                                                                                     W=self._img_intrinsics.width,
                                                                                     full_intrinsics = self._get_pseudo_intrinsics(self._img_intrinsics))
        return pred_depth, pred_rgb, render_outputs

    def _render_patch_with_intrinsics(self, intrinsics, pose, model, H, W, full_intrinsics,camera_info=None):
        # pseudo_intrinsics = (intrinsics.fx, intrinsics.fy, intrinsics.cx, intrinsics.cy)
        patch_rays = get_rays(poses=pose.unsqueeze(0), intrinsics=self._get_pseudo_intrinsics(intrinsics),
                              H=self._patch_size, W=self._patch_size, N=-1)
        center = (intrinsics.cx, intrinsics.cy)
        if camera_info is None:
            camera_info = convert2CamInfo(pose, full_intrinsics, H, W)
        # import pdb;pdb.set_trace()
        output = render_auto(camera_info,model,torch.tensor([0.,0.,0.],device=self._device))
        # outputs = model.render(patch_rays['rays_o'], patch_rays['rays_d'])
        pred_rgb = output["render"].permute(1,2,0)
        pred_depth = output["depth"].permute(1,2,0)
        # import pdb;pdb.set_trace()
        # pred_rgb = sample_patch_from_center(pred_rgb,center,self._patch_size,self._patch_size,epoch=1)
        # pred_depth = sample_patch_from_center(pred_depth,center,self._patch_size,self._patch_size,epoch=1)
        pred_rgb = sample_patch_from_img(rays_d=patch_rays['rays_d_cam'], img=pred_rgb,
                                           img_intrinsics=full_intrinsics, patch_size=self._patch_size)
        pred_depth = sample_patch_from_img(rays_d=patch_rays['rays_d_cam'], img=pred_depth,
                                           img_intrinsics=full_intrinsics, patch_size=self._patch_size)
        ...
        return pred_depth, pred_rgb, center, output

    def _sample_patch(self, image, image_intrinsics, pose, model,camera,epoch):
        # import pdb;pdb.set_trace()
        image = image.squeeze()
        H = image.shape[-3]
        W = image.shape[-2]
        intrinsics_tmp=Intrinsics(image_intrinsics[0],image_intrinsics[1],image_intrinsics[2],image_intrinsics[3],W,H)
        self._img_intrinsics=intrinsics_tmp
        patch_intrinsics = self._get_random_patch_intrinsics(intrinsics_tmp)
        rendered_depth, rendered_rgb, center, render_outputs = self._render_patch_with_intrinsics(
            intrinsics=patch_intrinsics, pose=pose, model=model, H = H, W = W, full_intrinsics = image_intrinsics,camera_info=camera
        )
        # print('pose', pose)
        with torch.no_grad():
            patch_rays = get_rays(poses=pose.unsqueeze(0), intrinsics=self._get_pseudo_intrinsics(patch_intrinsics),
                              H=self._patch_size, W=self._patch_size, N=-1)
            ...
            # gt_rgb = sample_patch_from_center(image,center,self._patch_size,self._patch_size,epoch=epoch)

        return rendered_depth, gt_rgb, render_outputs

    def _get_random_patch_intrinsics(self,intrinsics=None) -> Intrinsics:
        if intrinsics is None:
            intrinsics=self._full_image_intrinsics
...
def sample_patch_from_center(img,center,H=48,W=48,epoch=1):
    
    h, w, c = img.shape
    # grid_x, grid_y = torch.meshgrid(torch.arange(H),torch.arange(W))
    
    # grid_x = (w/2 + grid_x+center[0]-W/2+0.5)/w*2.-1.
    # grid_y = (h/2 + grid_y+center[1]-H/2+0.5)/h*2.-1.
    # grid = torch.cat([grid_y,grid_x]).reshape(1,H,W,2).to(img.device)
    
    # sampled = grid_sample(input=img[None,...].moveaxis(-1, 1), grid=grid, align_corners=True)
    # # import pdb;pdb.set_trace()
    # sampled = sampled.reshape(1,c,H,W)
    ...
    sampled=img[int(center[0]+w/2)-W//2:int(center[0]+w/2)+W//2,int(center[1]+h/2)-H//2:int(center[1]+h/2)+H//2,:][None,...]

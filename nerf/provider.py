...
class NeRFDataset:
    def __init__(self, opt, device, type='train', downscale=1, n_test=120):
        super().__init__()
    ...
    def collate(self, index):
        ...
        if self.rand_pose == 0 or index[0] >= len(self.poses):
            ...
            return_data = CameraInfo(uid=index[0], R=poses[0, :3, :3], T=poses[0, :3, 3], 
                            FovY=self.fov_y, FovX=self.fov_x,
        ...
        if self.images is not None:
            ...
            results['images_full'] = images.squeeze()
            if self.training:
                C = images.shape[-1]
    ...
    def get_CameraInfo(self) -> List[CameraInfo]:
        CamInfo_ls = []
        size = len(self.poses)
        if self.training and self.rand_pose > 0:
            size += size // self.rand_pose # index >= size means we use random pose.
        for idx in range(size):
            if self.rand_pose == 0 or idx >= len(self.poses):
                ...
                return_data = CameraInfo(uid=idx, R=poses[0, :3, :3], T=poses[0, :3, 3], 
                                FovY=self.fov_y, FovX=self.fov_x,
                                image=0, image_path='', image_name='', 
                                width=self.W, height=self.H)

                CamInfo_ls.append(return_data)
            else:
                poses = self.poses[[idx]].to(self.device) # [B, 4, 4]
